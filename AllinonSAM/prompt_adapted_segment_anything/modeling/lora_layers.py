...
class LoRALinear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, bias: bool = True, device=None, dtype=None, r=8, scale=1) -> None:
...
class LoRAConv2D(nn.Conv2d):
    def __init__(self, in_channels: int, out_channels: int, kernel_size, stride = 1, padding = 0, dilation = 1, groups = 1, bias = True, padding_mode: str = 'zeros', device=None, dtype=None, r=8, scale=1) -> None:
        super().__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode, device, dtype)
        assert type(kernel_size) is int
        self.r = r
        self.scale = 2*(r**0.5)
    ...
    def forward(self, input):
        out = F.conv2d(input, self.weight, self.bias, self.stride)
        out = out + self.scale*self.trainable_lora_up(self.selector(self.trainable_lora_down(input)))
        return out,0
